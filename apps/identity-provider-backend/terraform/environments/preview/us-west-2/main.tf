...
locals {
  ...
  pass_online_domain     = "passonline.dev"
  extra_oidc_subdomains  = ["barca", "jen"]
}

module "futureverse-oidc" {
  source = "../../../modules/futureverse-oidc"

  repository_id = "267797773052"
  image_tag     = var.image_tag

  # base config
  zone_name               = "futureverse.dev"
  domain_name             = "login.futureverse.dev"
  captcha_domain_name     = "login.futureverse.dev"
  ...
  oidc_hostname           = "https://login.futureverse.dev"
  sendgrid_from_email     = "verify@futureverse.cloud"
  sendgrid_template_id    = "d-78f913c0d8c7473fa9fe4e158134b745"
  foundation_api_base_url = "https://foundation.passonline.dev"
  logger_level            = "debug"
  oidc_cdn_record_weight  = local.oidc_cdn_record_weight

  cdn_hostname = "https://cdn.passonline.dev"

  signer_hostname = "https://signer.passonline.dev"
  ...
  # 3rd-party idps
  google_client_id = "913547680811-5n0fp27u2mfo6560bghsgkf3alq3oj3b.apps.googleusercontent.com"
  ...
  signer_legacy_redirect_uri = "https://signer.futureverse.dev/api/auth/callback/fv"
  signer_redirect_uri        = "https://signer.passonline.dev/api/auth/callback/fv"
  demo_client_id             = "demo"
  demo_redirect_uri          = "https://identity-demo.futureverse.dev/api/auth/callback/fv"
  dashboard_hostname         = "https://identity-dashboard.futureverse.dev"
  dashboard_client_id        = "dashboard"
  dashboard_redirect_uri     = "https://identity-dashboard.futureverse.dev/login,https://futureverse-identity-dashboard-*.vercel.app/login"
  ...
  # rate limit
  captcha_ip_rate_limit_threshold = 9998
