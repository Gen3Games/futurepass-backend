...
/**
 * Creates an instance of `TiktokStrategy`.
 *
...
function TiktokStrategy(options, verify) {
  ...
  if (!verify) {
    throw new TypeError('TiktokStrategy requires a verify callback')
  }

  if (!options.clientID) {
    throw new TypeError('TiktokStrategy requires a clientID option')
  }
  ...
  this._oauth2 = new OAuth2(
    ...
    '',
    tiktokUrls.authorizationURL,
    tiktokUrls.tokenURL,
    options.customHeaders
  )

  this._callbackURL = options.callbackURL
  this._profileURL = tiktokUrls.profileURL
  ...
  this._key =
    options.sessionKey ||
    'tiktok:' + url.parse(tiktokUrls.authorizationURL).hostname
  ...
  if (options.store && typeof options.store == 'object') {
    ...
    if (options.pkce) {
      throw new TypeError(
        'TiktokStrategy requires `state: true` option when PKCE is enabled'
      )
    }
...
util.inherits(TiktokStrategy, passport.Strategy)
 ...
 * @api protected
 */
TiktokStrategy.prototype.authenticate = function (req, options) {
  options = options || {}
  var self = this
  ...
  if ((req.query && req.query.code) || (req.body && req.body.code)) {
    ...
    if (this._pkceMethod) {
      verifier = utils.generateCodeVerifier(128)
      switch (this._pkceMethod) {
        case 'plain':
          challenge = verifier
          break
        case 'S256':
          challenge = SHA256(verifier).toString(CryptoJS.enc.Hex)
          break
        default:
          return this.error(
            new Error(
    ...
    if (state && typeof state == 'string') {
      ...
      parsed.query['client_key'] = this._oauth2._clientId
      ...
      function stored(err, state) {
        ...
        utils.merge(parsed.query, params)
        parsed.query['client_key'] = self._oauth2._clientId
        delete parsed.search
...
TiktokStrategy.prototype.userProfile = function (accessToken, done) {
  ...
  //these fields are based scope
  //refers to https://developers.tiktok.com/doc/tiktok-api-v2-get-user-info
  const allowedFields = ['open_id', 'avatar_url', 'display_name']

  var fields = '?fields=' + allowedFields.join(',')
  ...
  this._oauth2.get(url + fields, accessToken, function (err, body, res) {
    ...
    try {
      ...
      self._oauth2.useAuthorizationHeaderforGET(false)
    }
  })
}
...
TiktokStrategy.prototype.tokenParams = function (options) {
  return {
    client_key: this._oauth2._clientId,
  }
}
 ...
 * @api protected
 */
TiktokStrategy.prototype.parseErrorResponse = function (body, status) {
  var json = JSON.parse(body)
  if (json.error) {
 ...
 * @api private
 */
TiktokStrategy.prototype._loadUserProfile = function (accessToken, done) {
  var self = this
 ...
 * @api private
 */
TiktokStrategy.prototype._createOAuthError = function (message, err) {
  var e
