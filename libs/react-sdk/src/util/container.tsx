/**
 * FIXME: This is a duplication of the functionality in futureverse-stateboss to make it available for use within
 * this package. If we import directly from futureverse-stateboss we get an error when building demo-react-app.
 * The error appears to be due to discrepancies between the way the two packages are built. If we can resolve that
 * error this can go, if not we can consider shifting this functionality here permanently. However, this means
 * whoever consumes this package would potentially have access to it. This wouldn't be the worst as this abstraction
 * for react context creation is well tested and useful. Further discussion around the pros and cons of each option
 * is needed.
 */
      ...
      if (value === EMPTY) {
        throw new Error(
          `Component must be wrapped with <${
            options?.componentName ?? 'Container'
          }.Provider>`
        )
      }
      return value
